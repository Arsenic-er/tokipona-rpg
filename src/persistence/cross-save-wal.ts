...
export interface CrossSaveWalCompactReceipt {
  readonly transactionId: string;
  readonly transactionKind: string;
  readonly canonicalIdempotencyKey: string;
...
export interface CrossSaveWalSave {
  ...
  /** Absent only in legacy v0.1 saves written before payload collection shipped. */
  readonly compactReceipts?: readonly CrossSaveWalCompactReceipt[];
...
export interface DurableCrossSaveWalStore {
  ...
  reconcileFromSave(records: readonly CrossSaveWalRecord[], collectedTransactionIds?: readonly string[]): readonly CrossSaveWalRecord[];
}
...
export class CrossSaveWalDuplicateReceiptError extends Error {
  public constructor(public readonly receipt: CrossSaveWalCompactReceipt) {
    super("transaction payload was already collected; compact receipt is authoritative");
    this.name = "CrossSaveWalDuplicateReceiptError";
  }
}
...
const requestHashForEnvelopes = (envelopes: readonly CrossSaveWalOperationEnvelope[]): `sha256:${string}` => sha256Canonical(envelopes as unknown as JsonValue);
...
const compactReceiptValid=(value:unknown,contract:CrossSaveWalContract):value is CrossSaveWalCompactReceipt=>{
  if(!isRecord(value)||!nonEmpty(value.transactionId)||!nonEmpty(value.transactionKind)||!nonEmpty(value.canonicalIdempotencyKey)||!digest(value.requestHash)||!Array.isArray(value.receiptKinds)||!uniqueStrings(value.receiptKinds as string[])||!Array.isArray(value.deterministicReceiptIds)||!uniqueStrings(value.deterministicReceiptIds as string[])||!counter(value.collectedTick)||!contract.registeredTransactionKinds[value.transactionKind as string])return false;
  const kinds=value.receiptKinds as string[],ids=value.deterministicReceiptIds as string[];
  return value.transactionId===createCrossSaveTransactionId(value.transactionKind as string,value.canonicalIdempotencyKey as string)&&kinds.length===ids.length&&kinds.every((kind,index)=>ids[index]===createCrossSaveReceiptId(value.transactionId as string,kind));
};
...
const freezeCompactReceipt=(value:CrossSaveWalCompactReceipt):CrossSaveWalCompactReceipt=>Object.freeze({...value,receiptKinds:Object.freeze([...value.receiptKinds]),deterministicReceiptIds:Object.freeze([...value.deterministicReceiptIds])});
const compactFromRecord=(record:CrossSaveWalRecord,collectedTick:number):CrossSaveWalCompactReceipt=>freezeCompactReceipt({transactionId:record.transactionId,transactionKind:record.transactionKind,canonicalIdempotencyKey:record.canonicalIdempotencyKey,requestHash:requestHashForEnvelopes(record.operationEnvelopes),receiptKinds:record.operationEnvelopes.flatMap((envelope)=>envelope.receiptKinds),deterministicReceiptIds:record.operationEnvelopes.flatMap((envelope)=>envelope.deterministicReceiptIds),collectedTick});
const saveBody=(save:Omit<CrossSaveWalSave,"checksum">):JsonValue=>({schema:save.schema,contract:save.contract as unknown as JsonValue,records:save.records as unknown as JsonValue,...(save.compactReceipts===undefined?{}:{compactReceipts:save.compactReceipts as unknown as JsonValue}),receiptIndex:save.receiptIndex as unknown as JsonValue,acceptingNewTransactions:save.acceptingNewTransactions});
export const isCrossSaveWalSave=(value:unknown):value is CrossSaveWalSave=>{
  if(!isRecord(value)||value.schema!==CROSS_SAVE_WAL_SCHEMA||!contractValid(value.contract)||!Array.isArray(value.records)||!(value.records as unknown[]).every((record)=>recordValid(record,value.contract as CrossSaveWalContract))||(value.compactReceipts!==undefined&&(!Array.isArray(value.compactReceipts)||!(value.compactReceipts as unknown[]).every((receipt)=>compactReceiptValid(receipt,value.contract as CrossSaveWalContract))))||!Array.isArray(value.receiptIndex)||!uniqueStrings(value.receiptIndex as string[])||typeof value.acceptingNewTransactions!=="boolean"||!digest(value.checksum))return false;
  const save=value as unknown as CrossSaveWalSave,body={schema:save.schema,contract:save.contract,records:save.records,...(save.compactReceipts===undefined?{}:{compactReceipts:save.compactReceipts}),receiptIndex:save.receiptIndex,acceptingNewTransactions:save.acceptingNewTransactions};if(sha256Canonical(saveBody(body))!==save.checksum)return false;
  const compact=save.compactReceipts??[],transactionIds=[...save.records.map((r)=>r.transactionId),...compact.map((r)=>r.transactionId)],keys=[...save.records.map((r)=>r.canonicalIdempotencyKey),...compact.map((r)=>r.canonicalIdempotencyKey)];
  if(new Set(transactionIds).size!==transactionIds.length||new Set(keys).size!==keys.length)return false;
  const acknowledged=new Set([...save.records.flatMap((r)=>r.operationEnvelopes.filter((_,i)=>r.participantApplyAcks.includes(r.participants[i]!.saveOwner)).flatMap((e)=>e.deterministicReceiptIds)),...compact.flatMap((receipt)=>receipt.deterministicReceiptIds)]);return save.receiptIndex.every((id)=>acknowledged.has(id));
};

export class CrossSaveWalRuntime {
  private readonly participantByOwner:ReadonlyMap<string,CrossSaveWalParticipant>;private records:CrossSaveWalRecord[]=[];private compactReceipts:CrossSaveWalCompactReceipt[]=[];private receipts=new Set<string>();private accepting=true;private recoveryReady=true;private loadFailure:string|null=null;
  ...
  public begin(input:CrossSaveWalBeginInput):CrossSaveWalRecord{
    ...
    const transactionId=createCrossSaveTransactionId(input.transactionKind,input.canonicalIdempotencyKey),envelopes=this.createEnvelopes(transactionId,input.operations),existing=this.records.find((r)=>r.transactionId===transactionId||r.canonicalIdempotencyKey===input.canonicalIdempotencyKey),compact=this.compactReceipts.find((r)=>r.transactionId===transactionId||r.canonicalIdempotencyKey===input.canonicalIdempotencyKey);
    ...
    if(compact){if(compact.transactionKind!==input.transactionKind||compact.requestHash!==requestHashForEnvelopes(envelopes))throw new Error("idempotency key conflicts with different request");throw new CrossSaveWalDuplicateReceiptError(compact);}
  ...
  public garbageCollect(transactionId:string,tick:number):CrossSaveWalRecord{const record=this.requireRecord(transactionId);if(record.state!=="applied"||record.participantSnapshotAcks.length!==record.participants.length)throw new Error("all durable participant snapshots are required");const collected=this.replace(record,{state:"garbage_collectable",updatedTick:tick},"garbage_collectable"),compact=compactFromRecord(collected,tick);this.compactReceipts=[...this.compactReceipts.filter((entry)=>entry.transactionId!==transactionId),compact];this.records=this.records.filter((entry)=>entry.transactionId!==transactionId);return collected;}
  public snapshot():CrossSaveWalSave{const core={schema:CROSS_SAVE_WAL_SCHEMA,contract:this.options.contract,records:Object.freeze(this.records.map(freezeRecord)),receiptIndex:Object.freeze([...this.receipts].sort()),acceptingNewTransactions:this.accepting}as const,base=this.compactReceipts.length===0?core:{...core,compactReceipts:Object.freeze(this.compactReceipts.map(freezeCompactReceipt))};return Object.freeze({...base,checksum:sha256Canonical(saveBody(base))});}
  public load(save:unknown):void{this.accepting=false;this.recoveryReady=false;if(!isCrossSaveWalSave(save)){this.loadFailure="WAL save is corrupt";throw new Error(this.loadFailure);}if(canonicalJson(save.contract as unknown as JsonValue)!==canonicalJson(this.options.contract as unknown as JsonValue)){this.loadFailure="WAL contract mismatch";throw new Error(this.loadFailure);}this.loadFailure=null;try{const savedCompact=save.compactReceipts??[],reconciled=this.options.durableStore.reconcileFromSave(save.records,savedCompact.map((entry)=>entry.transactionId));if(!reconciled.every((record)=>recordValid(record,this.options.contract)))throw new Error("durable WAL store returned corrupt record");const migrated=reconciled.filter((record)=>record.state==="garbage_collectable").map((record)=>compactFromRecord(record,record.updatedTick)),compactById=new Map<string,CrossSaveWalCompactReceipt>();for(const entry of [...savedCompact,...migrated]){const prior=compactById.get(entry.transactionId);if(prior&&canonicalJson(prior as unknown as JsonValue)!==canonicalJson(entry as unknown as JsonValue))throw new Error("compact receipt conflicts with durable WAL");compactById.set(entry.transactionId,freezeCompactReceipt(entry));}this.compactReceipts=[...compactById.values()];this.records=reconciled.filter((record)=>record.state!=="garbage_collectable").map(freezeRecord);this.receipts=new Set([...save.receiptIndex,...this.compactReceipts.flatMap((entry)=>entry.deterministicReceiptIds),...this.records.flatMap((record)=>record.operationEnvelopes.filter((_,index)=>record.participantApplyAcks.includes(record.participants[index]!.saveOwner)).flatMap((envelope)=>envelope.deterministicReceiptIds))]);}catch(error){this.loadFailure=error instanceof Error?error.message:"durable WAL store failure";throw error;}}
  public recordsSnapshot():readonly CrossSaveWalRecord[]{return Object.freeze(this.records.map(freezeRecord));}
  public compactReceiptsSnapshot():readonly CrossSaveWalCompactReceipt[]{return Object.freeze(this.compactReceipts.map(freezeCompactReceipt));}
