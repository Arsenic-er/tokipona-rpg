...
function validateSource(
  ...
  switch (source.kind) {
    ...
    case "region":
      ...
      if (source.path === "data/world/regions/valley-prologue.v0.1.yaml" &&
          readString(source.content, "region_id") === "valley_prologue") {
        validateForestContinuousMapContract(source, issues);
      }
      break;
    case "ecology":
...
function validateReturnFlowTaskReferences(source: CompiledSource, sources: readonly CompiledSource[], sceneSource: CompiledSource | undefined, region: ContentObject | undefined, issues: ContentIssue[]): void {
  ...
  if (!sceneSource || readString(sceneSource.content, "scene_id") !== "scene.valley.return_channel" || readString(indicator??{},"target_kind")!=="inert_return_flow_mechanism" || !sameStringArray(targets, ["return_flow.inert_force_indicator", "return_flow.overflow_gate", "return_flow.mud_blockage", "return_flow.old_channel", "return_flow.split_flow_gauge", "return_flow.return_spout", "return_wetland.large_creature.nest_trace", "return_wetland.large_creature.young_trace", "return_wetland.large_creature.migration_channel", "return_wetland.large_creature.food_scent_guide", "return_wetland.large_creature.nonlethal_barrier"])) addIssue(issues, "task.return_flow_scene", source.path, "scene_ref", "N07 scene identity and eleven canonical return-flow and large-creature wetland targets are required");
...
function validateForestContinuousMapContract(source: CompiledSource, issues: ContentIssue[]): void {
  const expected: ContentObject = {
    schema_version: "g01.forest-spatial.v0.1", runtime_origin: "top_left",
    region_bounds_px: { width: 10240, height: 2880 }, viewport_px: { width: 640, height: 360 },
    viewport_envelope: { columns: 16, rows: 8 }, visible_material_cell_px: 1,
    ...
    chapter_one_route_anchor_ids: ["forest.arrival", "forest.stream", "forest.settlement", "forest.hermit_branch", "forest.waterwheel", "forest.cistern", "forest.return_channel", "forest.underground_node", "forest.settlement"],
    ...
    water_course_control_points_px: [[1088, 672], [1792, 800], [3328, 752], [4352, 1120], [5184, 1504], [7424, 1792]],
    landmarks: [{ landmark_id: "forest.waterwheel_structure", district_id: "forest.waterwheel", bounds_px: [4800, 1120, 1408, 1024], reveal_stage_ids: ["channel_sound", "support_beams", "broken_rim", "inner_machinery"] }],
    encounter_chambers: [
  ...
  if (!sameForestContractValue(source.content.continuous_map_contract, expected)) {
    addIssue(issues, "region.continuous_forest_spatial_contract", source.path, "continuous_map_contract", "continuous forest spatial contract is missing, unknown, or noncanonical");
  }
}

function sameForestContractValue(actual: ContentValue | undefined, expected: ContentValue): boolean {
  if (actual === expected) return true;
  if (Array.isArray(actual) || Array.isArray(expected)) {
    return Array.isArray(actual) && Array.isArray(expected) && actual.length === expected.length &&
      actual.every((value, index) => sameForestContractValue(value, expected[index]!));
  }
  if (isContentObject(actual) || isContentObject(expected)) {
    if (!isContentObject(actual) || !isContentObject(expected)) return false;
    const actualKeys = Object.keys(actual).sort();
    const expectedKeys = Object.keys(expected).sort();
    return actualKeys.length === expectedKeys.length && actualKeys.every((key, index) => key === expectedKeys[index] && sameForestContractValue(actual[key], expected[key]!));
  }
