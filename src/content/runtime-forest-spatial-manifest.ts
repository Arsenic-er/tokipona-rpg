...
function validateCanonical(raw: Record<string, unknown>): void {
  ...
  const expectedDistrictScenes = ["scene.valley.arrival_shelf", "scene.valley.stream_section", "scene.valley.settlement", "scene.valley.stream_section", "scene.valley.waterwheel", "scene.valley.high_cistern", "scene.valley.den_bypass", "scene.valley.return_channel", "scene.valley.underground_order_node", "scene.valley.safe_range", "scene.valley.old_mine_threshold"];
  ...
  districts.forEach((entry, index) => { exactKeys(entry, ["districtId", "sceneId", "boundsPx"], "forest district"); if (entry.sceneId !== expectedDistrictScenes[index]) throw new Error("forest district scene mapping is noncanonical"); if (!sameRect(entry.boundsPx, expectedBounds[index]!)) throw new Error("forest district geometry is noncanonical"); });
