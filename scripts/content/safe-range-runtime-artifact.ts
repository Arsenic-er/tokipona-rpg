...
const TARGET_IDS = ["wood_dummy", "sandbag", "minecart", "hanging_stone", "material_collision_table"] as const;
const INTERACTION_IDS = ["safe_range.test_wood_dummy", "safe_range.test_sandbag", "safe_range.test_minecart", "safe_range.test_hanging_stone", "safe_range.inspect_material_collision_table"] as const;
const PROMPT_LEVELS = ["H0", "H1"] as const;
const MATERIAL_TARGET_CLASSES = ["wood_dummy", "sandbag", "minecart", "hanging_stone"] as const;
const REPAIR_TARGET_NODE_IDS = ["use.motion.noncombat", "use.intensity.inert"] as const;
const FORBIDDEN_INPUTS = ["kill_count", "wildlife_harm", "elapsed_real_time", "repeated_cast_count", "currency", "streak"] as const;
const PARALLEL_RECEIPT_IDEMPOTENCY_FIELDS = ["player_save_id", "action_id", "normalized_variant_hash"] as const;
const UNRELATED_ACTIONS = [
  ["settlement.calibration.unrelated_delivery_commit", "calibration.unrelated.delivery", "delivery_committed"],
  ["settlement.calibration.unrelated_route_commit", "calibration.unrelated.route", "route_committed"],
] as const;
...
export function projectSafeRangeQualification(manifest: ContentManifest): RuntimeSafeRangeManifest {
  ...
  const parallelActions = objects(parallel.actions, "parallel calibration actions").map((action) => ({
    ...
    authoritySceneId: oneOf(action.authority_scene_id, ["scene.valley.settlement", "scene.valley.return_channel"] as const, "parallel authority scene"),
    ...
    evidenceType: oneOf(action.evidence_type, ["active_retrieval", "noncombat_action", "noncombat_intensity", "repair", "delayed_retrieval"] as const, "parallel evidence type"),
    ...
    promptLevel: promptLevel(action.prompt_level),
  ...
  const unrelatedActions = objects(parallel.unrelated_semantic_world_actions, "parallel unrelated actions").map((action, index) => {
    const expected = UNRELATED_ACTIONS[index];
    if (!expected) throw new Error(`unexpected unrelated semantic action ${index}`);
    return {
      actionId: exactString(action.action_id, expected[0], "unrelated action id"), authoritySceneId: exactString(action.authority_scene_id, "scene.valley.settlement", "unrelated authority scene"), authorityTaskId: exactString(action.authority_task_id, expected[1], "unrelated authority task"), taskFamilyId: exactString(action.task_family_id, "settlement_calibration_context", "unrelated family"), outcome: exactString(action.outcome, expected[2], "unrelated outcome"), qualificationEvidence: exactBoolean(action.qualification_evidence, false, "unrelated evidence"),
    };
  });
  if (unrelatedActions.length !== UNRELATED_ACTIONS.length || new Set(unrelatedActions.map((action) => action.actionId)).size !== unrelatedActions.length) throw new Error("unrelated semantic action IDs must be unique and complete");
  ...
  const body = {
    ...
    scene: {
      ...
      targetIds: exactStrings(task.target_ids, TARGET_IDS, "target_ids"), interactionIds: exactStrings(task.interaction_ids, INTERACTION_IDS, "interaction_ids"), materialTableInteractionPointTiles: exactNumberPair(required(targetMap, "material_collision_table").interaction_point_tiles, [20, 1] as const, "material collision table interaction point"), interactionBindings, livingTargetCount: objects(scene.targets, "targets").filter((target) => string(target.target_kind, "target_kind").includes("living")).length as 0,
    },
    ...
    prerequisiteGraph: {
      ...
      }, completionEvent: exactString(graph.completion_event, "attack_capacity_calibrated", "graph completion"), forbiddenInputs: exactStrings(graph.forbidden_inputs, FORBIDDEN_INPUTS, "graph forbidden inputs"),
    },
    signature: {
    ...
    parallelCalibration: {
      ...
      receipt: { receiptRequired: exactBoolean(parallelReceipt.receipt_required, true, "parallel receipt required"), idempotencyKeyFields: exactStrings(parallelReceipt.idempotency_key_fields, PARALLEL_RECEIPT_IDEMPOTENCY_FIELDS, "parallel idempotency fields"), duplicateEvidenceAwardForbidden: exactBoolean(parallelReceipt.duplicate_evidence_award_forbidden, true, "parallel duplicate evidence") },
  ...
  return { sourceDigest: computeRuntimeManifestDigest(body), ...body };
}
...
function exactStrings<const T extends readonly string[]>(value: unknown, expected: T, label: string): T { const actual = strings(value, label); if (actual.length !== expected.length || actual.some((item, index) => item !== expected[index])) throw new Error(`${label} is noncanonical`); return expected; }
function oneOf<const T extends readonly string[]>(value: unknown, expected: T, label: string): T[number] { const actual = string(value, label); if (!expected.includes(actual as T[number])) throw new Error(`${label} is invalid`); return actual as T[number]; }
...
function promptLevel(value: unknown): 0 | 1 { if (value === 0 || value === 1) return value; throw new Error("parallel prompt level must be 0 or 1"); }
...
function exactNumberPair<const T extends readonly [number, number]>(value: unknown, expected: T, label: string): T { const pair = numberPair(value, label); if (pair[0] !== expected[0] || pair[1] !== expected[1]) throw new Error(`${label} must be [${expected.join(",")}]`); return expected; }
