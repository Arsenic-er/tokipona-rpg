...
审计后批准的三小时剧情、7+2 地点、媒介碎片、隐士教学和双段高潮见[《森林第一章〈水往何处〉设计规格》](../superpowers/specs/2026-08-28-forest-chapter-design.md)。其内容权威（数据契约、生成运行时投影与跨域集成门）已实现；这不表示游戏玩法已经完成。运行时 reducer／Flow、保存与重放接线、正式 UI、美术资产和端到端验收仍待后续运行时实施计划明确后再进入开发。
